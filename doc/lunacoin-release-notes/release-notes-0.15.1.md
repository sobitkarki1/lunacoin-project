Lunacoin Core version *0.15.1* is now available from:

  <https://download.lunacoin.org/lunacoin-0.15.1/>
...
A number of changes to the way Lunacoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 milliluna per kB (0.00001 LNC). This is to prepare for dropping the minimum transaction fee to 0.1 milliluna per kB (0.0001 LNC) in the next release.
