...
*Setup instructions for a Gitian build of Lunacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lunacoin
...
- [Building Lunacoin Core](#building-lunacoin-core)
...
Clone the git repositories for lunacoin and Gitian.
...
in the lunacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Lunacoin Core
----------------

To build Lunacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lunacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lunacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lunacoin-project/lunacoin
    From https://github.com/lunacoin-project/lunacoin
...
For example:
```bash
URL=https://github.com/thrasher-/lunacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lunacoin=${COMMIT} --url lunacoin=${URL} ../lunacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lunacoin=${COMMIT} --url lunacoin=${URL} ../lunacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lunacoin=${COMMIT} --url lunacoin=${URL} ../lunacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lunacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lunacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lunacoin
SIGPATH=/some/root/path/lunacoin-detached-sigs

./bin/gbuild --url lunacoin=${BTCPATH},signature=${SIGPATH} ../lunacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lunacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lunacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lunacoin-osx-build.assert
```
...
[lunacoin-project/gitian.sigs.ltc](https://github.com/lunacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
[lunacoin-project/gitian.sigs.ltc](https://github.com/lunacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
