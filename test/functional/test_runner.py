...
def main():
    ...
    tmpdir = "%s/lunacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and lunacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "lunacoind"]) is not None:
            print("%sWARNING!%s There is already a lunacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "LUNACOIND" not in os.environ:
        os.environ["LUNACOIND"] = build_dir + '/src/lunacoind' + exeext
        os.environ["LUNACOINCLI"] = build_dir + '/src/lunacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `lunacoin-cli help` (`rpc_interface.txt`).
